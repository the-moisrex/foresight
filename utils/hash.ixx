...
namespace fs8 {
    ...
    [[nodiscard]] constexpr uint32_t ci_hash(std::basic_string_view<CharT> const src) noexcept {
        ...
        for (auto code_point : src) {
            ...
            hash ^= static_cast<uint32_t>(code_point);
            hash *= FNV1A_32_PRIME;
        }
        return hash;
    }

    export void fnv1a_init(uint32_t& hash) noexcept;
    export void fnv1a_hash(uint32_t& hash, char32_t uch) noexcept;
