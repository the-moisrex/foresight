...
using snd_pcm_open_fn       = int (*)(snd_pcm_t**, const char*, snd_pcm_stream_t, int);
using snd_pcm_close_fn      = int (*)(snd_pcm_t*);
using snd_pcm_set_params_fn = int (*)(snd_pcm_t*, snd_pcm_format_t,
                                       snd_pcm_access_t, unsigned int,
                                       unsigned int, int, unsigned int);
using snd_pcm_writei_fn     = snd_pcm_sframes_t (*)(snd_pcm_t*, const void*, snd_pcm_uframes_t);
...
using snd_strerror_fn       = const char* (*)(int);
...
struct alsa_api {
    ...
    bool load() noexcept {
        ...
        return pcm_open != nullptr
            && pcm_writei != nullptr
            && pcm_set_params != nullptr;
    }
};
...
struct alsa_backend final : audio_backend {
    ...
    void do_on_ready() noexcept {
        ...
        while (written < frames) {
            auto const rc = api().pcm_writei(pcm,
                buf.data() + written * queue_channels,
                frames - written);
...
fs8::audio_backend_result fs8::detail::try_alsa() noexcept {
    ...
    if (b->start()) {
        ...
        return {
            .backend = std::move(b),
            .on_ready = [](void* ctx) noexcept { static_cast<alsa_backend*>(ctx)->do_on_ready(); },
            .on_ready_ctx = raw
        };
    }
#endif
