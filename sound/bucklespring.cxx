    ...
    for (std::size_t i = 0; i < frames; ++i) {
        ...
        if (has_attack) {
            ...
            if (src_idx < entry.frames) {
                ...
                float a0 = static_cast<float>(bucklespring_attack_blob[entry.offset + src_idx * 2]
                                            | (bucklespring_attack_blob[entry.offset + src_idx * 2 + 1] << 8));
                // int16 → float: reinterpret bits
                auto const i0 = static_cast<int16_t>(
                    bucklespring_attack_blob[entry.offset + src_idx * 2]
                    | (bucklespring_attack_blob[entry.offset + src_idx * 2 + 1] << 8));
                a0 = static_cast<float>(i0) / 32768.0f;

                float a1 = 0.0f;
                if (src_idx + 1 < entry.frames) {
                    auto const i1 = static_cast<int16_t>(
                        bucklespring_attack_blob[entry.offset + (src_idx + 1) * 2]
