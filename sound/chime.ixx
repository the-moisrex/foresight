...
export namespace fs8 {
    ...
    /// Parameterized synthesizer for bell-like chime keystroke sounds.
    ///
    /// Each key has its own voice parameters (center frequency, bandwidth,
    /// envelope shape, ...) extracted from WAV analysis.
    /// The synthesis is noise-driven through a biquad bandpass resonator
    /// with a dual-exponential amplitude envelope.
    struct [[nodiscard]] chime_synth {
        constexpr chime_synth() noexcept = default;

        // -- raw interface (keycode + pressed) ------------------------------

        /// Look up the voice parameters for a given keycode and press state.
        [[nodiscard]] constexpr chime_voice const& voice(uint8_t const keycode, bool const pressed) const noexcept {
            auto const& table = pressed ? chime_press_params : chime_release_params;
