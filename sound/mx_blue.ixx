...
export module fs8.mods:mx_blue;
...
export namespace fs8 {
    ...
    /// Synthesizer for Cherry MX Blue (click-jacket) keystroke sounds.
    ///
    /// The archetype loud tactile click: a sharp jacket tick on the way down,
    /// a brighter, faster tick on the way up, over a firm bottom-out thock.
    /// Parameter tables are measured per-key from a reference recording set.
    /// Pure synthesis through the shared click engine
    /// (`fs8::detail::render_click`) — no recorded attack samples.
    struct [[nodiscard]] mx_blue_synth {
        constexpr mx_blue_synth() noexcept = default;
