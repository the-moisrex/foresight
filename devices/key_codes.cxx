...
namespace {
    ...
    struct case_insensitive_less_t {
        template <typename CharT>
        [[nodiscard]] constexpr bool operator()(keynames_type const& lhs, std::basic_string_view<CharT> rhs) const noexcept {
...
namespace {
    template <typename CharT>
    [[nodiscard]] uint16_t key_code_of_impl(std::basic_string_view<CharT> name) noexcept {
        using fs8::keynames;
...
template <>
uint16_t fs8::key_code_of<char32_t>(std::basic_string_view<char32_t> const name) noexcept {
    return key_code_of_impl(name);
}

template <>
uint16_t fs8::key_code_of<char16_t>(std::basic_string_view<char16_t> const name) noexcept {
    return key_code_of_impl(name);
}

template <>
uint16_t fs8::key_code_of<char8_t>(std::basic_string_view<char8_t> const name) noexcept {
    return key_code_of_impl(name);
}

template <>
uint16_t fs8::key_code_of<char>(std::basic_string_view<char> const name) noexcept {
    return key_code_of_impl(name);
}
