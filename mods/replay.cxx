...
context_action fs8::basic_replay::operator()(event_type& event, special_event const& tag) noexcept {
    ...
    return st_->is_binary ? handle_load_binary(event) : handle_load_text(event);
}

// ── Private helpers ──────────────────────────────────────────────────────────

context_action fs8::basic_replay::handle_start() noexcept {
    using enum context_action;
    ensure_state();
...
context_action fs8::basic_replay::handle_load_binary(event_type& event) noexcept {
    using enum context_action;
...
context_action fs8::basic_replay::parse_buffered_lines(event_type& event) noexcept {
    using enum context_action;
    while (true) {
...
context_action fs8::basic_replay::handle_load_text(event_type& event) noexcept {
    using enum context_action;
