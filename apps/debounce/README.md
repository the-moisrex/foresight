...
occasionally double click), but it can be pointed at any event code.

## Usage

```bash
./debounce [device] [-t <time>] [-c <codes>]
./debounce -h | --help
```
...
```bash
./debounce keyboard --codes KEY_A,KEY_D,KEY_F,KEY_J
```

Quiet a noisy tablet axis (only settled changes pass):

```bash
./debounce tablet --codes EV_ABS:ABS_X,EV_ABS:ABS_Y
```

Drop double-fired scroll-wheel steps:

```bash
./debounce mouse --codes EV_REL:REL_WHEEL
```
